...
const file = "sheets-client.js";
const logger = require('../helpers/logger.js')('sheets_client');
  ...
  get_scraper(retailer_name) {
    ...
    switch (retailer_name) {
      ...
      case 'AMAZON':
        ...
        break;
      default:
        logger.error(`${file} get_scraper: No scraper found for ${retailer_name}.`);
    }
    return scraper;
  }
  make_request(obj) {
    return new Promise((resolve, reject) => {
      request.get(obj.url).set(obj.retailer.headers).end(function(err, resp) {
        if (err) logger.error(`${file} make_request: ${err.message}`);
        if (resp && resp.status) {
          logger.info(`${file} make_request: PRODUCT: ${obj.product}`);
          logger.info(`${file} make_request: REQ to ${obj.retailer.name}`);
          logger.info(`${file} make_request: RES STATUS ${resp.status}`);
          if (resp.res.text.toLowerCase().includes('robot')) {
            logger.warn(`${file} make_request: Robot detected`);
          }
          if (resp.status >= 400) {
            logger.error(`${file} make_request: ${resp.res.text}`);
            resolve([]);
          } else {
          ...
          logger.error(`${file} make_request: ${obj.product}`, resp);
          resolve([]);
        }
      }.bind(this));
    });
    ...
    return new Promise((resolve, reject) => {
      ...
      each(costco_objs, function(costco_obj, next) {
        ...
        this.get_data_from_retailers(costco_obj.Name).then(data => {
          logger.verbose(`${file} scrape_retailers: Extracted data from retailers`, data);
        })
        if (count % 10 == 0) {
          setTimeout(function() {
            next();
  ...
  from_costco_to_CSV() {
    ...
    new CostcoProducts().get_products().then(data => {
      ...
      require('fs').writeFile(export_path, csv, err => {
        if (err) {
          logger.error(`${file} from_costco_to_CSV: Error writing file`, err);
          throw err
        }
        logger.info(`${file} from_costco_to_CSV: File costco-products.csv saved to ${export_path}`);
      });
    });
  }
}
