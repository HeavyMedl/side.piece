...
class SheetsClient extends SheetsAPI {
	constructor(id) {
		...
		this.retailers = {
			ebay: {
				...
				protocol: "http://",
				host: "www.ebay.com",
				path: "/sch/i.html?_from=R40&_trksid=p2050601.m570.l1313.TR0.TRC0.H0.X.TRS2&_nkw=keywords&_sacat=0"
			},
			amazon: {
				name: "AMAZON",
				protocol: "http://",
		...
		return new Promise((resolve, reject) => {
			request.get(obj.url)
				.end(function(err, resp) {
					...
					if (resp && resp.status) {
						console.log(chalk.black.bgWhite(obj.product));
						console.log(chalk.gray('| REQ |') + ` to ${obj.retailer.name}`);
						console.log(chalk.cyan('| RES |') + ` STATUS ${this.status(resp.status)}\n`);
						if (resp.status >= 400) {
							console.log(resp.res.text);
							resolve(resp);
						} else {
							// resolve scraped objects
						}
					} else {
	...
	get_url(retailer, keywords) {
		let r = this.retailers[retailer];
		let url = `${r.protocol}${r.host}${r.path.replace('keywords', encodeURIComponent(keywords))}`;
		return url;
	}
	scrape_retailers(costco_objs) {
		return new Promise((resolve, reject) => {
			...
			each(costco_objs, function(costco_obj, next) {
				count++;
				this.make_request({
						url: this.get_url('ebay', costco_obj.Name),
						product: costco_obj.Name,
						retailer: this.retailers.ebay
					})
					.then(resp => {
						// hit up amazon
					})
