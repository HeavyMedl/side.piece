...
class SheetsClient extends SheetsAPI {
	...
	make_request(obj) {
		request.get(obj.url)
			.end(function(err, resp) {
				...
				if (resp && resp.status) {
					console.log(`${obj.count}. ` + chalk.black.bgWhite(obj.costco_obj.Name));
					...
					console.log(`${obj.count}. ` + chalk.black.bgWhite(obj.Name));
					console.log(resp + '\n');
				}
	...
	scrape_retailer(costco_objs, retailer) {
		return new Promise((resolve, reject) => {
			let len = costco_objs.length,
				count = 1;
	...
	test(csv) {
		this.get_objs_from_CSV(csv)
			.then(objs => this.scrape_retailer(objs))
			.then(data => {
